...
class PublicSurfaceContractTests(unittest.TestCase):
    def test_single_country_registry(self):
        countries = list_exposed_countries(["denmark"], allow_internal=False)
        self.assertEqual(len(countries), 1)
        self.assertEqual(countries[0].country_id, "denmark")
    ...
    def test_country_config_matches_public_surface(self):
        ...
        self.assertEqual(config.display_name, "Denmark")
        self.assertEqual(config.adjective, "Danish")
        self.assertEqual(len(config.supported_factors), 16)
    ...
    def test_public_wrapper_defaults(self):
        ...
        self.assertIn('WPD_APP_TITLE", "Danish Politics Data"', wrapper)
        self.assertIn('WPD_EXPOSE_COUNTRIES", "denmark"', wrapper)
